...
class jdkHandeler:
    ...
    def get_jdk_client(self, game_version=float):
        ...
        if not os.path.exists(ConfigHandeler.Minecraft_Dir+"/jdks"):
            os.mkdir(ConfigHandeler.Minecraft_Dir+"/jdks")

        if game_version <= 116.5:
            try:
                return os.path.normpath(glob.glob(ConfigHandeler.Minecraft_Dir+'/jdks/jdk'+jdk_version+'*')[0])
            except:
                jdk.install(version=jdk_version,
                            operating_system=jdk.OperatingSystem.detect(),
                            arch=jdk.Architecture.detect(),
                            jre=True,
                            path=os.path.normpath(ConfigHandeler.Minecraft_Dir+'/jdks'),
                            vendor="Adoptium")
                return os.path.normpath(glob.glob(ConfigHandeler.Minecraft_Dir+'/jdks/jdk'+jdk_version+'*')[0])
        else:
            try:
