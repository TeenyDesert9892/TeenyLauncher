...
def main(page: ft.Page):
    ...
    def modify_versions(event=None):
        version, version_list = instancesHandeler.check_versions(modifyInstancesType.value)
        for i, ver in enumerate(version_list):
            version_list[i] = ft.dropdown.Option(ver)
    ...
    def modify_versions_engines(event=None):
        engine, engine_list = instancesHandeler.check_engine_ver(modifyInstancesVersion.value, modifyInstancesType.value)
        for i, eng in enumerate(engine_list):
            engine_list[i] = ft.dropdown.Option(eng)
    ...
    def modify_version_start(event=None):
        Message(instancesHandeler.modify_instance(modifyInstancesType.value,
    ...
    def start_instance_install(event=None):
        Message(instancesHandeler.install_instance(addInstanceName.value,
    ...
    def start_instance_uninstall(event=None):
        Message(instancesHandeler.uninstall_instance(removeInstancesDropdown.value,
                                                     setStatus,
                                                     setProgress,
                                                     setMax))
        progressReset()
    ...
    def modify_instance_name_updater(event=None):
        if modifyInstancesInstance.value != lang.Without_Versions:
            modifyInstancesName.value = config.Instances[modifyInstancesInstance.value]["Name"]
            if event:
...
def run_web():
    global runned
    runned = True
    ft.app(main, assets_dir=".", view=None, port=9892, host="0.0.0.0")
    config.save_config()
